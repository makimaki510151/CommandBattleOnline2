...
import { passiveAbilities, endTurnPassiveAbilities, specialAbilityConditions, skillEffects, damagePassiveEffects, criticalPassiveEffects, normalAttackPowerMultiplier } from './character_abilities.js';
...
window.executeAction = (data) => {
    ...
    const resolveForcedTargetUniqueId = (chosenTargetUniqueId) => {
        ...
        // 2) 選択ターゲットが「かばう」状態（ランパート等）：味方のみ。敵のtaunt(プロヴォーク)は誘導に使わない
...
function performAttack(attacker, target) {
    ...
    let power = 1.0;
    if (normalAttackPowerMultiplier[attacker.originalId]) {
        power = normalAttackPowerMultiplier[attacker.originalId](attacker);
    }
    const { damage, critical, dodged } = calculateDamage(attacker, target, false, power, false, 'single');
    ...
    combatants.forEach(c => {
        ...
        Object.keys(c.effects || {}).forEach(effectName => {
            ...
            if (e && e.duration !== undefined) {
                e.duration--;
                if (e.duration <= 0) {
                    const disp = { guarding: '護衛', taunt: '護衛', orderDelay: '行動遅延', spdDebuff: '素早さ低下', critRateBuff: '会心率上昇', defDebuff: '防御低下', mdefDebuff: '魔防低下', atkDebuff: '攻撃低下', matkDebuff: '魔攻低下', dodgeDebuff: '回避低下', dodgeBuff: '回避上昇' }[effectName] || effectName;
